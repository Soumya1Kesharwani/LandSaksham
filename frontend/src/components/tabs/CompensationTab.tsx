      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase text-slate-500">Total Statutory Award (RFCTLARR 2013)</span>
            <Landmark className="w-4 h-4 text-gov-navy" />
          </div>
          <div className="text-2xl font-bold font-mono text-slate-900 mt-2">
            ₹{totalCompensation.toFixed(2)} Crore
          </div>
          <p className="text-xs text-slate-500 mt-1">Includes 100% Solatium & 12% statutory interest</p>
        </div>

        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase text-emerald-800">Disbursed (DBT / Bank Transferred)</span>
            <CheckCircle2 className="w-4 h-4 text-emerald-600" />
          </div>
          <div className="text-2xl font-bold font-mono text-emerald-950 mt-2">
            ₹{paidCompensation.toFixed(2)} Crore ({paidPct}%)
          </div>
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase text-red-800">Pending / Escrowed Balance</span>
            <AlertCircle className="w-4 h-4 text-red-600" />
          </div>
          <div className="text-2xl font-bold font-mono text-red-950 mt-2">
            ₹{pendingCompensation.toFixed(2)} Crore
          </div>
          <p className="text-xs text-red-700 mt-1">Blocked by court stays & mutation verification holds</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 text-white rounded-lg p-5 shadow-sm space-y-3">
        <div className="flex items-center justify-between border-b border-slate-800 pb-2">
          <h3 className="text-sm font-bold text-amber-400 uppercase tracking-wide">
            Statutory Valuation Matrix (RFCTLARR Act 2013 & State Rules)
          </h3>
          <span className="text-xs bg-slate-800 px-2 py-0.5 rounded text-slate-300">
            Form 23 Standardized Award
          </span>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div className="bg-slate-800/80 p-3 rounded border border-slate-700">
            <span className="text-slate-400 block text-[11px]">Rural Multiplier Factor</span>
            <strong className="text-white text-sm">1.25x – 2.0x</strong>
            <span className="text-[10px] text-slate-400 block">Applied to DLC / Circle rate</span>
          </div>
          <div className="bg-slate-800/80 p-3 rounded border border-slate-700">
            <span className="text-slate-400 block text-[11px]">Section 30 Solatium</span>
            <strong className="text-emerald-400 text-sm">100% Mandatory</strong>
            <span className="text-[10px] text-slate-400 block">100% of market value grant</span>
          </div>
          <div className="bg-slate-800/80 p-3 rounded border border-slate-700">
            <span className="text-slate-400 block text-[11px]">Section 30(3) Interest</span>
            <strong className="text-amber-400 text-sm">12% Per Annum</strong>
            <span className="text-[10px] text-slate-400 block">From Section 4 notification</span>
          </div>
          <div className="bg-slate-800/80 p-3 rounded border border-slate-700">
            <span className="text-slate-400 block text-[11px]">R&R Resettlement Package</span>
            <strong className="text-blue-400 text-sm">₹5.0 Lakh / House</strong>
            <span className="text-[10px] text-slate-400 block">Or replacement pucca house</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-200 bg-slate-50/50 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2">
            ...
            <input
              type="text"
              placeholder="Search by Parcel ID, Khasra, Owner or Village..."
              value={searchTerm}
          ...
          <button
            onClick={() => alert("PFMS Treasury Export Generated in XLSX format.")}
          ...
          >
            ...
            <span>Export PFMS Treasury Ledger</span>
          </button>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse font-mono">
            <thead className="bg-slate-100 text-slate-700 font-sans font-semibold border-b border-slate-200">
              <tr>
                ...
                <th className="p-3 text-right">Base Land Value</th>
                <th className="p-3 text-right">100% Solatium</th>
                ...
                <th className="p-3 font-sans">Payment Status</th>
                <th className="p-3 font-sans text-right">Action</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={p.id} className="hover:bg-blue-50/40 transition">
                    <td className="p-3 font-bold text-gov-navy">
                      <div>{p.id}</div>
                      <div className="text-[11px] font-normal text-slate-500">Khasra {p.khasra_survey_no} ({p.village})</div>
                    </td>
                    <td className="p-3 font-sans text-slate-800">
                      {p.owner.name}
                    </td>
                    <td className="p-3 text-right text-slate-700">
                      ₹{(comp.base_land_value_inr / 100000).toFixed(2)} L
                    </td>
                    <td className="p-3 text-right text-slate-700">
                      ₹{(comp.solatium_100_pct_inr / 100000).toFixed(2)} L
                    </td>
                    <td className="p-3 text-right font-bold text-slate-900">
                      ₹{(comp.total_estimated_compensation_inr / 100000).toFixed(2)} L
                    </td>
                    <td className="p-3 text-right text-emerald-700 font-semibold">
                      ₹{(comp.amount_disbursed_inr / 100000).toFixed(2)} L
                    </td>
                    <td className="p-3 text-right text-red-700 font-bold">
                      ₹{(comp.amount_pending_inr / 100000).toFixed(2)} L
                    </td>
